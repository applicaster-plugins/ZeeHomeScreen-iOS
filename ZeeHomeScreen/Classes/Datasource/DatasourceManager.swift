    ...
    public func parse(data: NSObject?, componentModel: ComponentModel?, isParentModel: Bool) -> ComponentModelProtocol? {
      var components = [ComponentModelProtocol]()
        ...
        for (index, entry) in entries.enumerated() {
            ...
            //parse entries inside component but don't parse BANNER component and RECOMMENDATIONS, becaus banner is a single component without entries
            if !entry.isBannerType(), let entry = entry as? APAtomContainerProtocol, let extensions = entry.extensions, extensions["related_collections"] == nil  {
